# Guide for connecting to the CSLAB Kubernetes environment and running Apache Spark

## Introduction

This guide provides detailed instructions for connecting to the CSLAB lab infrastructure through VPN, installing the required tools, preparing a local workstation, and running Apache Spark jobs on Kubernetes (k8s).
...
After connecting to the VPN, access from Windows usually works without issues. On some WSL Linux installations, however, DNS-related settings may not be configured automatically. Open your WSL Linux environment and run:

```bash
ping kubernetes
```

If everything is configured correctly, you should see output similar to the following (stop it with `Ctrl+C`, otherwise it keeps running). Here we can see that `kubernetes` resolves correctly to IP `10.233.0.1`.
...
```bash
# Install basic packages required for HTTPS repository access
...
- `--master k8s://https://termi7.cslab.ece.ntua.gr:6443`: defines the Kubernetes master endpoint. The `k8s://` prefix indicates that the job will run on Kubernetes. `termi7.cslab.ece.ntua.gr:6443` is the address of the Kubernetes API server.
...
These parameters allow a Spark application to run correctly on a Kubernetes cluster while also configuring HDFS access, logs, executors, and other Kubernetes-related runtime settings.
...
```text
Enter: Save the file using the same directory / name
```
